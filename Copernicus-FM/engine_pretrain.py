...
WV = {
    ...
    "s2_toa": [443,490,560,665,705,740,783,842,865,945,1610,2190],
    "s3_olci": [412.5, 442.5, 490, 510, 560, 620, 665, 673.75,  
                708.75, 753.75, 761.25, 764.375, 767.5, 778.75, 865, 885, 900, 940, 1029],
    "s5p_co": [2325],
    "s5p_no2": [430],
    "s5p_o3": [300],
    "s5p_so2": [320],
    ...
    "sar1":[225000000],
    "sar2":[50000000],
    "guang":[485,555,660,830]
}
# bandwidths (in nm)
BW = {
    "s1_grd": [1e9, 1e9],
    "s2_toa": [20, 65, 35, 30, 15, 15, 20, 115, 20, 20, 90, 180],
    "s3_olci": [10, 10, 10, 10, 10, 10, 10, 7.5, 
                7.5, 10, 7.5, 7.5, 3.75, 2.5, 15, 20, 10, 10, 20],
    "s5p_co": [80],
    "s5p_no2": [60],
    "s5p_o3": [30],
    "s5p_so2": [15],
...
AREA = {
    ...
    "sar1": (KERNEL_SIZE["sar1"] * GSD["sar1"] / 1000) ** 2,
    "sar2": (KERNEL_SIZE["sar2"] * GSD["sar2"] / 1000) ** 2,
    "guang": (KERNEL_SIZE["guang"] * GSD["guang"] / 1000) ** 2
...
def prepare_metadata(meta, device):
    ...
    sample_meta = {
        ...
        "guang": meta[:, 8, :].to(device, non_blocking=True),
        "sar1": meta[:, 9, :].to(device, non_blocking=True),
        "sar2": meta[:, 10, :].to(device, non_blocking=True)
    }
...
def train_one_epoch(model: torch.nn.Module,
    ...
    for data_iter_step, samples in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        #TODO 
        sample_s1, sample_s2, sample_s3, sample_co, sample_no2, sample_o3, sample_so2, sample_dem, sample_sar1, sample_sar2, sample_guang, meta = samples
        ...
        samples_new = {
            ...
            "s2_toa": train_transforms['s2_toa'](sample_s2.to(device, non_blocking=True)),
            ...
            "dem": train_transforms['dem'](sample_dem.to(torch.float32).to(device, non_blocking=True)),
            "sar1": train_transforms['sar1'](sample_sar1.to(device, non_blocking=True)),
            "sar2": train_transforms['sar2'](sample_sar2.to(device, non_blocking=True)),
            "guang":train_transforms['guang'](sample_guang.to(device, non_blocking=True))
        }
        ...
        if args.distill_size is not None:
            ...
            samples_new["rgb"] = samples_new["guang"][:, [2,1,0], :, :] # Bx3x224x224
            WV["rgb"] = WV["guang"][0:3][::-1] # r,g,b
            BW["rgb"] = BW["guang"][0:3][::-1]
            KERNEL_SIZE["rgb"] = KERNEL_SIZE["guang"]
            sample_meta["rgb"] = sample_meta["guang"]
