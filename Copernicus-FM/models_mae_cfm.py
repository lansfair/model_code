...
import sys
sys.path.insert(0,"/mnt/ht2-nas2/00-model/00-ds/Copernicus-FM/h5towds/Copernicus-FM/src")
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def initialize_weights(self):
        ...
        wg_weights = torch.load('/mnt/ht2-nas2/00-model/00-ds/Copernicus-FM/Copernicus-FM/weights/weight_generator_1000_0.01_er50k.pt', map_location=device)
        ...
        # 定义本地 DINOv2 仓库路径
        local_dinov2_path = '/mnt/ht2-nas2/00-model/00-ds/dinov2'
        
        if self.distill_size == 'base':
            # source='local' 告诉 torch.hub 从本地目录加载 hubconf.py
            self.teacher = torch.hub.load(local_dinov2_path, 'dinov2_vitb14', source='local', trust_repo=True)
            # self.teacher = torch.hub.load('/mnt/ht2-nas2/00-model/00-ds/dinov2','dinov2_vitb14',source='local',trust_repo=True)
        elif self.distill_size == 'large':
            self.teacher = torch.hub.load(local_dinov2_path, 'dinov2_vitl14', source='local')
        elif self.distill_size == 'giant':
            self.teacher = torch.hub.load(local_dinov2_path, 'dinov2_vitg14', source='local')
        # 如果 distill_size 是 'none'，则不加载 teacher 模型
    ...
    def forward(self, samples, wvs, bws, metas, drop_prob=0.3, mask_ratio=0.75, kernel_size=None):
        ...
        for key in samples.keys():
            ...
            ks = kernel_size[key]
            #TODO 
            input_mode = 'spectral' if key in ['s1_grd', 's2_toa', 's3_olci', 'rgb', 'sar1', 'sar2','guang'] else 'variable'
