...
def build_common_components(
    ...
    config.training_modalities = [
        ...
        Modality.SAR.name,
        Modality.LT1.name,
        Modality.LANDCOVER_1M.name,
        Modality.LANDCOVER_30M.name
    ]
    return config
...
def build_dataset_config(common: CommonComponents) -> OlmoEarthDatasetConfig:
    ...
    return OlmoEarthDatasetConfig(
        ...
        # h5_read_cache_size=0,  # 禁用旧的 FIFO 缓存，由滑动窗口预加载替代
        # preload_window_size=5000,  # 每窗口 500 样本 × 6MB ≈ 3GB，2 窗口 ≈ 6GB/worker
    )
