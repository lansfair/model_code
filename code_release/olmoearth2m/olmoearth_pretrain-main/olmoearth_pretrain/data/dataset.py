...
class OlmoEarthDataset(Dataset):
    ...
    def __getitem__(self, args: GetItemArgs) -> tuple[int, OlmoEarthSample]:
        ...
        这是Dataset类最重要的方法,被DataLoader频繁调用。
        
        完整处理流程 (约0.08秒/样本):
        
        1️⃣ **索引映射** (第1149-1152行):
           - 将逻辑索引args.idx映射到实际的文件索引
           - sample_indices数组可能已过滤掉无效样本
        
        2️⃣ **文件读取** (第1153-1154行):
           - 调用read_h5_file从HDF5加载原始数据
           - 包含缓存、限流等优化
        
        3️⃣ **时间裁剪** (第1156-1158行):
           - 移除前后无效的时间步
           - 减少后续计算量
        
        4️⃣ **时间填充** (第1160行):
           - 将timestamps扩展到max_sequence_length
           - 记录原始长度current_length用于子集采样
        
        5️⃣ **缺失值填充** (第1162-1164行):
           - 为缺失的模态创建全MISSING_VALUE数组
           - 为部分缺失的时间步填充MISSING_VALUE
           - ⚠️ 这是最耗时的步骤之一
        
        6️⃣ **空间/时间子集采样** (第1166-1183行):
           - 根据token_budget限制,随机裁剪空间patch和时间窗口
           - 支持两种策略:
             * subset_sample_default: 矩形裁剪
             * subset_sample_cutmix: CutMix增强(随机拼接不同位置的patch)
           - 确保输出token数不超过模型上下文窗口
        
        7️⃣ **特征工程** (第1189-1196行):
           - 如果配置了NDVI,从Sentinel-2原始波段计算
           - 这是一个派生模态,不存储在HDF5中
        
        8️⃣ **数据归一化** (第1198-1215行):
           - 对所有非缺失模态进行标准化
           - ⚠️ 关键: 缺失模态不归一化,保持MISSING_VALUE不变
           - 归一化后恢复缺失值的标记,以便模型识别
        
        返回值:
        - patch_size: 用于后续tokenization
        - OlmoEarthSample: 完整的样本对象,包含所有模态
        
        性能瓶颈:
        - read_h5_file: I/O密集,可通过cache_dir优化
        - fill_sample_with_missing_values: CPU密集,约0.08秒
        - normalize_image: 如果有大量模态,也可能较慢
        """
