...
class OlmoEarthDataLoader(DataLoaderBase):
    ...
    def get_global_indices(self) -> np.ndarray:
        ...
        self._global_indices = np.memmap(self._global_indices_file, mode="r", dtype=np.uint32)
        return self._global_indices
    ...
    def _get_mock_sample(self, rng: np.random.Generator) -> OlmoEarthSample:
        ...
        if Modality.RGB.name in self.dataset.training_modalities:
            ...
            # Shape: [H, W, T, C]
            mock_rgb = rng.random(
                (standard_hw, standard_hw, 12, 4), dtype=np.float32
            )
        ...
        if Modality.SAR.name in self.dataset.training_modalities:
            ...
            # Shape: [H, W, T, C]
            mock_sar = rng.random(
                (standard_hw, standard_hw, 12, 1), dtype=np.float32
            )
...
class _IterableDatasetWrapper(torch.utils.data.IterableDataset[OlmoEarthSample]):
    ...
    def __iter__(self) -> Iterator[Any]:
        ...
        # preloader = None
        # if (
        #     isinstance(dataset, OlmoEarthDataset)
        #     and hasattr(dataset, "_preload_window_size")
        #     and dataset._preload_window_size > 0
        # ):
        #     preloader = _SlidingWindowPreloader(
        #         dataset=dataset,
        #         indices=indices_array,
        #         window_size=dataset._preload_window_size,
        #     )
        #     # 异步预加载第一个窗口，传入 worker_id 实现错峰加载
        #     # start() 会在 get_next_bytes() 首次调用时自动触发
        #     worker_id = self.worker_info.id if self.worker_info is not None else 0
        #     num_workers = self.worker_info.num_workers if self.worker_info is not None else 1
        #     preloader.preload_first_window(worker_id=worker_id, num_workers=num_workers)
        #     dataset._preloader = preloader

        # try:
        #     # 创建从数据集获取样本的迭代器（此期间第一个窗口在后台异步加载）
        #     instance_iterator = (
        #         self.data_loader._get_dataset_item(int(idx), patch_size, sampled_hw_p)
        #         for idx, patch_size, sampled_hw_p in self._get_batch_item_params_iterator(
        #             indices_array,
        ...
        #         self.data_loader.drop_last,
        #     ):
        #         yield self.data_loader.collator(batch)  # type: ignore[arg-type]
        # finally:
        #     # epoch 结束时清理预加载器
        #     if preloader is not None:
        #         preloader.stop()
        #         dataset._preloader = None
