...
def train(config: OlmoEarthExperimentConfig) -> None:
    ...
    # 在并行化之前加载普通.pth
    if getattr(config, "pretrained_weight_path", None):
        logger.info(f"Loading init weights from: {config.pretrained_weight_path}")
        ckpt = torch.load(config.pretrained_weight_path, map_location="cpu", weights_only=False)
        state_dict = ckpt.get("model", ckpt)

        incompatible = model.load_state_dict(state_dict, strict=False)
        logger.info(
            f"Loaded init weights. "
            f"missing_keys={len(incompatible.missing_keys)}, "
            f"unexpected_keys={len(incompatible.unexpected_keys)}"
        )
        if incompatible.missing_keys:
            logger.warning(f"Missing keys (first 20): {incompatible.missing_keys[:20]}")
        if incompatible.unexpected_keys:
            logger.warning(f"Unexpected keys (first 20): {incompatible.unexpected_keys[:20]}")
