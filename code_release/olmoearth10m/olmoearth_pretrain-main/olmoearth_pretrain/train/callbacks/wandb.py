...
class OlmoEarthWandBCallback(WandBCallback):
    ...
    def pre_train(self) -> None:
        ...
        if self.enabled and get_rank() == 0:
            ...
            csv_path = Path(self.trainer.save_folder) / "metrics.csv"
    ...
    def log_metrics(self, step: int, metrics: dict[str, float]) -> None:
        ...
        if get_rank() == 0 and getattr(self, "_csv_file", None) is not None:
            if not hasattr(self, "_csv_buffered_step"):
                self._csv_buffered_step: int = step
                self._csv_step_buffer: dict[str, float] = {}
            if step != self._csv_buffered_step:
                self._flush_csv_row()
                self._csv_buffered_step = step
                self._csv_step_buffer = {}
            self._csv_step_buffer.update(metrics)

    def _flush_csv_row(self) -> None:
        if not getattr(self, "_csv_step_buffer", None):
            return
        row = {"step": self._csv_buffered_step, **self._csv_step_buffer}
        if self._csv_writer is None:
            write_header = self._csv_file.tell() == 0
